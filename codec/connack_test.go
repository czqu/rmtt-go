...
func TestConnackPacket_Write(t *testing.T) {
	...
	expected := []byte{Connack << 4, 3, Accepted, 0x00, 0x3C}
...
func TestConnackPacket_Unpack(t *testing.T) {
	...
	if ca.ServerKeepalive != 60 {
		t.Errorf("Unpack() ServerKeepalive = %v, want 60", ca.ServerKeepalive)
	}
}
